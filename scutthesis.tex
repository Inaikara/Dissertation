...
% \includeonly{section/7_研究成果}
% \includeonly{section/8_致谢}
...
\hypersetup{pdftitle={基于观测输入的机器人模仿学习研究},
	pdfauthor={匿名}, % 可能会泄露作者信息，盲审时请注意
	pdfsubject={华南理工大学硕士学位论文}, % 博士自行修改
	pdfkeywords={模仿学习；高斯混合模型；动态运动原语；扩散策略；视觉-语言-动作模型},
...
\begin{document}
	...
	\include{section/1_绪论}
	\include{section/2_预备知识}
	\include{section/3_正文}
	\include{section/4_正文}
	\include{section/5_正文}

	\backmatter %章节不编号但页码继续
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    微调，使得后续章节的页眉不带章号
	\renewcommand{\chaptermark}[1]{\markboth{#1}{}}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\include{section/6_总结与展望} %结论
	 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% bibtex参考文献设置  （原版）
...
%%	\bibliography{F:/MyLibrary}
	...
	% \includepdf[pages=1]{cover_file/papers.pdf} %盲审成果
	\include{section/7_研究成果} %成果
	\include{section/8_致谢} %致谢
	% \includepdf[pages=1]{cover_file/答辩决议书.pdf}
\end{document}